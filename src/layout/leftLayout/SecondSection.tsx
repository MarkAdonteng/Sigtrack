...
const SecondSection: React.FC<SecondSectionProps> = ({ children }) => {
  ...
  return (
    <div
      className={`second-section  ${isNarrowed ? 'w-18' : 'w-[40px]'}  ${isNarrowed ? 'h-[845px]' : 'h-[75px]'}  z-20 ml-16 mt-[-0.5rem] absolute flex justify-center  items-center w-80 mr-auto bg-gray-300 text-alternate-text transition-all duration-100 ease-in-out rounded-lg`}
    >
      <div className="content-wrapper">
        <button
          className={`arrow-button1 ml-48 text-2xl mt-3 text-gray-500 transform transition-transform duration-100 ease-in-out absolute border-none
